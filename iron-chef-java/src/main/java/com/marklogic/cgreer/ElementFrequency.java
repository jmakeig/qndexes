...
public class ElementFrequency {
			...
			if (key != null && value != null && value.get() != null) {
				try {
					...
					while (iterator.hasNext()) {
						...
						String hadoopKey2 = key+elementKey+"-pc";
						//log.info(e.getName());
						...
						if (e.getParentElement() != null) {
							log.info("Found parent element to do something");
							...
							pcElement = pcElement.setAttribute("ns", e.getNamespaceURI());
							pcElement = pcElement.setAttribute("localname", e.getName());
							pcElement = pcElement.setAttribute("parent-ns", e.getParentElement().getNamespaceURI());
							pcElement = pcElement.setAttribute("parent-localname", e.getParentElement().getName());
							pcElement = pcElement.addContent(new org.jdom.Text(getKey(e.getParentElement()) + elementKey));
							
							baos = new ByteArrayOutputStream();
							out.output(pcElement, baos);
							outputString = baos.toString();
							log.info(outputString);
							context.write(new Text(hadoopKey2), new Text(outputString));
						}
	...
	public static class ElementFrequencyReducer extends
		...
		public void reduce(Text key, Iterable<Text> values,
			...
			if (e.getAttributeValue("parent-ns") != null) {
				e2.setAttribute("parent-ns", e.getAttributeValue("parent-ns"));
				e2.setAttribute("parent-localname",e.getAttributeValue("parent-localname"));				
			}
